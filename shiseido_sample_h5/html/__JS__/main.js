...
//序列帧动画
var canvas1 = new createjs.Stage(document.getElementById('canvas1'));
function box1Show() {
  // canvas1.audioOut.unlock();
  $('.loading').fadeOut();
  createjs.Ticker.setFPS(10);
  createjs.Ticker.addEventListener("tick", handleTick);
  var pageIndex = 0;
  function handleTick(event) {
    var item1 = new createjs.Bitmap(queue.getResult("canvas_item1_" + pageIndex));
    canvas1.removeAllChildren();
    canvas1.addChild(item1);
    canvas1.update();
    pageIndex++;
    // console.log(pageIndex)
    if (pageIndex == 122) {
      swiper.slideToLoop(1, 700, true)
      music.pause();
...
$('.box_1 .button').click(function () {
  ...
  swiper.slideToLoop(2, 1, true);//切换到第一个slide，速度为1秒
  music.pause();
})
